...
from .api_config import qwen_chat_completions_url
...
class LLMClient:
    ...
    def __init__(
        ...
        self.hf_base_url = hf_base.rstrip("/") if hf_base else "https://router.huggingface.co/v1"
        ...
        configured_gemini_mode = (
            gemini_api_mode or os.environ.get("GEMINI_API_MODE", "")
        ).strip().lower()
        if configured_gemini_mode and configured_gemini_mode not in {"google", "openai"}:
            raise ValueError("GEMINI_API_MODE must be either 'google' or 'openai'.")
        if configured_gemini_mode:
            self.gemini_api_mode = configured_gemini_mode
        elif self.gemini_api_base.rstrip("/").endswith("/v1") and "generativelanguage.googleapis.com" not in self.gemini_api_base:
            self.gemini_api_mode = "openai"
        else:
            self.gemini_api_mode = "google"
    ...
    def _call_qwen(
        ...
        try:
            url = qwen_chat_completions_url(self.qwen_base_url)
        except ValueError as exc:
            raise RuntimeError(f"DashScope API 地址配置错误: {exc}") from exc
        headers = {
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                hint = MODEL_HINTS.get("qwen", "")
                response_excerpt = (response.text or "").strip()[:500] or "<empty body>"
                endpoint_hint = ""
                if response.status_code == 404:
                    endpoint_hint = (
                        " 请确认 Base URL 使用 /compatible-mode/v1，而不是原生接口 /api/v1。"
                    )
                raise RuntimeError(
                    f"通义千问请求失败 (status={response.status_code})，模型: {self.model_name}。{hint}\n"
                    f"请求地址: {url}\n响应片段: {response_excerpt}{endpoint_hint}"
                )
    ...
    def _call_gemini(
        ...
        if self.gemini_api_mode == "openai":
            return self._call_gemini_openai_compatible(
                messages,
                prompt,
                call_index,
                pipeline_attempt,
    ...
    def _call_gemini_openai_compatible(
        self,
        messages: List[Dict[str, Any]],
        prompt: str,
        call_index: int,
        ...
        """Call a third-party Gemini service exposing OpenAI Chat Completions."""
        if OpenAI is None:
            raise RuntimeError("openai 库缺失: 请执行 pip install openai")

        model_id = (self.gemini_model or self.model_name).strip()
        ...
        # Per-call third-party plans may charge every retry. Keep both SDK
        # retries and streaming disabled so one pipeline attempt is one request.
        client = OpenAI(
            api_key=self.gemini_api_key,
        ...
        try:
            ...
            log_entry["error"] = str(exc)
            raise RuntimeError(
                f"调用 OpenAI-compatible Gemini 模型 {model_id} 失败: {exc}"
            ) from exc
        ...
        log_entry["result"] = "success"
        log_entry["response_text"] = response
        log_entry["temperature"] = DEFAULT_TEMPERATURE
        try:
            log_entry["raw_response"] = completion.model_dump_json()
        except Exception:
            log_entry["raw_response"] = str(completion)
    ...
    def _call_anthropic(
        ...
        if not api_key:
            hint = MODEL_HINTS.get("anthropic", "")
            raise RuntimeError(f"缺少 ANTHROPIC_API_KEY，无法调用 Anthropic 模型。{hint}")
        ...
        raise RuntimeError(
            f"Anthropic 模型 {self.model_name} 在 {max_attempts} 次请求后仍失败；"
            "已停止本次生成，避免将兜底动画误记为模型结果。"
        )
