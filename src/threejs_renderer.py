    ...
    def _setup_threejs_assets(self) -> None:
        ...
        if not main_path.exists():
            ...
            main_path.write_text(
                ...
                "    args: process.env.K12SIMWORLD_BROWSER_GPU === 'off'\n"
                "      ? ['--no-sandbox','--disable-setuid-sandbox','--disable-gpu','--enable-unsafe-swiftshader','--allow-file-access-from-files']\n"
                "      : ['--no-sandbox','--disable-setuid-sandbox','--enable-gpu','--ignore-gpu-blocklist','--enable-unsafe-swiftshader','--allow-file-access-from-files']\n"
                "  });\n"
                "  try {\n"
                "    const page = await browser.newPage();\n"
                "    let pageError = null;\n"
                "    page.on('pageerror', err => { pageError = err; console.log(`[PAGE ERROR] ${err.message}`); });\n"
                "    page.on('error', err => { pageError = err; console.log(`[ERROR] ${err.message}`); });\n"
